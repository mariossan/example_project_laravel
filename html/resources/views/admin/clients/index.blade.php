...
@section('js')
    <script>
        $(function() {
            ...
            *
            */
            var dataToTable     = {!! json_encode($clients) !!};
            var urlToSend       = "{{ url('/') }}";
             ...
             * */
             let printTable  = (dataToTable, filtro = null ) => {
                let dataToPrint = dataToTable.filter(function (simpleData, index, array) {
                    if ( filtro !== null ) {
                        let client_code     = simpleData.client_code.toUpperCase();
                        let business_name   = simpleData.business_name.toUpperCase();
                        let fiscal_name     = simpleData.fiscal_name.toUpperCase();

                        if ( client_code.includes(filtro) ) {
                            return true;

                        } else if ( business_name.includes(filtro) ) {
                            return true;

                        } else if( fiscal_name.includes(filtro) ) {
                            return true;
                        }
                    } else {
                        return true;
             ...
             let formatTable = ( dataToPrint ) => {
                ...
                if ( dataToPrint.length > 0 ) {
                    ...
                    for ( dataItem of dataToPrint ) {
                        let html = `<tr>
                                            <td>${ dataItem.client_code }</td>
                                            <td>${ dataItem.business_name }</td>
                                            <td>${ dataItem.fiscal_name }</td>
                                            <td align="right">
                                                <a href="${ urlToSend }/clients/${ dataItem.id }/edit" class="btn btn-sm btn-outline-success">
                                                    <span class="material-icons">edit</span>
                                                </a>

                                                <a href="${ urlToSend }/clients/${ dataItem.id }/delete" data-user='user${ dataItem.id }' class="btn btn-sm btn-outline-danger delItem" data-name='${ dataItem.business_name }'> <span class="material-icons">delete</span>
                                                </a>
